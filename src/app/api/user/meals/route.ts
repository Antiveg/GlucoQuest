...
import { createMealWithFoodsService, getUserMealsWithFoodsByUserIdService } from "@/lib/services/meal/meal-food-services";
...
export async function POST(request : NextRequest){
    ...
    try {
        ...
        const response = await createMealWithFoodsService(session.user.id, body);
        return NextResponse.json(response, { status: 200 });
    }catch (error: unknown) {
        console.error("GET /api/user/meals error:", error);
        return NextResponse.json(
